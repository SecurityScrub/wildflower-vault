    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/admin/calendar" className="btn-secondary py-2 px-4 text-xs">
            Today
          </Link>
          <Link
      ...
      <div className="bg-white overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b border-gray-100">
          {DAY_NAMES.map((d) => (
            <div key={d} className="px-3 py-2 font-sans text-xs uppercase tracking-wider text-gray-400 text-center">
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          ...
          {Array.from({ length: startDayOfWeek }).map((_, i) => (
            <div key={`empty-${i}`} className="border-b border-r border-gray-50 min-h-[100px] p-2 bg-gray-50/30" />
          ))}
          ...
          <div className="divide-y divide-gray-50">
            {bookings.map((b) => (
              <Link key={b.id} href={`/admin/orders?id=${b.id}`} className="flex items-center justify-between p-5 hover:bg-gray-50">
                <div>
                  <p className="font-sans font-semibold text-sm">{b.guestName}</p>
